...
with open("logos_ascii/carotte.txt", 'r') as f:
    carotte = f.read()
liste_des_etapes: Tuple[Tuple[int, list[str], float, Optional[int]], ...] = (
    ...
    (3, ["À saveur d'orange, il est au Popsicle® ce que le Revello® est au Fudge®"], 1, None),#ORANGE/BLANC
 ...
 \ |\/
  \|/
   `""",
    ...
    (7,["Les robot du futur seront-ils nu comme des vers ou auront ils une peau de PVC?"], 1, None), # Sans gaine
    (8, ["Mercenaire!", "armez", "uniquement", "vos", "ennemis!"], 1, None),#MAUVE
...
""",
     ...
     (15, ["Vous saurez bientôt s'il y en a au bout du tunnel.", "Et s'il n'en avait pas?"],1, None),# Noir
     (16, ["e = > , l = $, o = ), r = @",
           "b = a , u = %,  i = +",
           "a = & , t = ?, n = *",
     ...
     (18, ["Vu votre niveau d'intelligence, n'essayez pas le piano c'est trop compliqué pour vous, ça vous fera saigner du nez."],1,None), #Blanc noir et rouge
(19, ["Heureusement, il y a des gens qui ne saigne pas du nez lorsqu'ils en jousseent."], 1, None),
(20, ["Une facile en échange de vous verrez bien quoi...","Le vert avec des taches noires et blanches"], 1, None), # VERT blanc noir
...
class SequenceDesarmorcement:
    def __init__(self, fonction_mise_a_jour):
        ...
        self.__pins = [Pin(5, Pin.IN, Pin.PULL_UP)
                       for i in range(len(liste_des_etapes))]
    ...
    def __gpio_interrup_callback(self, pin: Pin, pin_number: int):
        ...
        if self.__cable_coupes & pin_mask:
